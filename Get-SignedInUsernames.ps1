...
foreach ($Computer in $Computers) {
	if (Test-Connection -BufferSize 32 -Count 1 -ComputerName $Computer -Quiet) {
		...
		$SignInCheck = quser.exe /Server:"$Computer" 2>$null # 2>$null supresses error output
		if ($SignInCheck) {
			...
			# Do something with them here before the next computer loop runs, for example...
   			"Signed in users:`n$UserNames"
		}
		else {
